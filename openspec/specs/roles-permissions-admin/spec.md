# roles-permissions-admin Specification

## Purpose

Administration of platform profiles (roles) and their CASL permissions via API and UI under Administración Global → Roles y permisos.
...
- Show "Ver" when `ability.can('read', 'RolePermission')` and "Editar" when `ability.can('update', 'RolePermission')`, following the same action pattern as platform users list.
...
Page `frontend/src/pages/RoleDetailPage.jsx` SHALL support `mode` `view` or `edit`:

- **view**: readonly fields, PermissionMatrix read-only, no save buttons; subheader "Editar" when `ability.can('update', 'RolePermission')`.
- **edit**: editable label, PermissionMatrix editable, save name/permissions, delete role when `usersCount === 0`.

Breadcrumb: Administración Global → Roles y permisos → [role label or Ver].

#### Scenario: Permissions saved triggers re-fetch

- **WHEN** the user saves permissions successfully in edit mode
...
- `admin-global/roles-permisos/:id/edit` → `RequireCan I="update" a="RolePermission"` → `RoleDetailPage mode="edit"`
- `admin-global/roles-permisos/:id` → `RequireCan I="read" a="RolePermission"` → `RoleDetailPage mode="view"`
