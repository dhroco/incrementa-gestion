...
Eliminate profiles `CONTADOR` and `USUARIO_EMPRESA_ADMINISTRADOR` from the platform. Only `ADMINISTRADOR_PLATAFORMA` remains as the business profile, with platform users managed via `/api/platform/users`.
## Requirements
