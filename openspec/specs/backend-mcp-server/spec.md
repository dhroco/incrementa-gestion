...
`backend/package.json` SHALL include scripts `"mcp": "node mcp.mjs"` and `"mcp:http": "node mcp-http.mjs"` for local smoke testing outside Claude Desktop.
...
#### Scenario: Run MCP HTTP via npm

- **WHEN** the developer runs `npm run mcp:http` in the backend directory with required env vars set
- **THEN** the MCP HTTP server starts and listens on the configured port
...
`registerMcpTools` SHALL accept `gcsService` in its dependencies object. `createMcpServer()` in `backend/mcpServer.mjs` SHALL pass the same `gcsService` instance used by `createDocumentBuilderService`.
...
#### Scenario: Factory reuses singleton db

- **WHEN** `createMcpServer()` is called multiple times in the same process
- **THEN** each call returns a distinct `McpServer` instance sharing the same Knex `db` singleton

### Requirement: MCP HTTP entrypoint with Streamable HTTP transport

The backend SHALL provide `backend/mcp-http.mjs` as a standalone Express process (not mounted on `app.js`) that exposes MCP at path `/mcp` using `StreamableHTTPServerTransport` from `@modelcontextprotocol/sdk/server/streamableHttp.js` in **stateless** mode (`sessionIdGenerator: undefined`). Each `POST /mcp` request SHALL create a new `McpServer` and `StreamableHTTPServerTransport`, connect them, delegate via `transport.handleRequest(req, res, req.body)` with `express.json()` parsing, and close both on `res.on('close')`. In stateless mode, `GET /mcp` and `DELETE /mcp` SHALL return HTTP 405 Method Not Allowed. The process SHALL listen on `process.env.PORT` (default 8080) and host `0.0.0.0`.
...
- **WHEN** two consecutive MCP HTTP POST requests arrive at `/mcp`
...
#### Scenario: Stateless GET and DELETE return 405

- **WHEN** a client sends `GET /mcp` or `DELETE /mcp` to the stateless MCP HTTP process
...
- **WHEN** a client sends a valid Streamable HTTP MCP POST request to `/mcp` without an Authorization header
