...
`documentBuilderService.js` MUST define a static extensible map `VARIABLE_META` keyed by placeholder name. Each entry MUST include at least `label` (Spanish string) and `type` (`text`, `date`, or `select`). A function `getVariableMeta(key)` MUST return the map entry or default `{ label: key, type: 'text' }` for unknown keys. Initial entries MUST include:

- `client_product_campaign`: `{ label: 'Producto/Campaña', type: 'select' }`
- `contract_date`: `{ label: 'Fecha del contrato', type: 'date' }`
- `signing_city`: `{ label: 'Ciudad de firma', type: 'text' }`

#### Scenario: Known variable returns typed metadata

- **WHEN** `getVariableMeta('contract_date')` is called
- **THEN** it returns `{ label: 'Fecha del contrato', type: 'date' }`
...
When `generateAndPersist` detects unresolved template placeholders, it MUST return HTTP-equivalent status 422 with code `MISSING_PLACEHOLDERS` and `data.missingFields` as an array of objects `{ key, label, type, options? }`. Each object MUST be built from `getVariableMeta(key)`. The response MUST NOT include `missingFieldKeys`. For `key === 'client_product_campaign'`, when `clientRow.product_campaigns` has at least one item, the field MUST have `type: 'select'` and `options` equal to the array of campaign `name` strings.

#### Scenario: Missing fields include metadata

- **WHEN** generation runs with unresolved `signing_city` and `contract_date`
...
- **THEN** the corresponding missing field has `type: 'select'` and `options` equal to those names
...
- `select` with `options` → select element listing options

The **Generar** button MUST be disabled until every field in the current `missingFields` list has a non-empty value in `missingFieldOverrides`. When dry-run succeeds (`ok: true`), the UI MUST indicate readiness to generate without showing missing-field inputs.
...
- **WHEN** missing fields include `contract_date` and the user has not set its override
