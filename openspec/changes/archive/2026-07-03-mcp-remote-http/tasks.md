...
- [x] 1.3 Smoke stdio: `npm run mcp` con env vars — proceso arranca sin error
...
- [x] 3.2 Verificar tests existentes de MCP (`mcpServer.test.js`) siguen verdes tras refactor de factory
- [x] 3.3 Ejecutar `npm test` en backend — suite completa verde (204/204)
...
- [x] 5.1 Local: `source set-env-local.sh && node mcp-http.mjs` — `/health` → 200, `GET /mcp` → 405
- [x] 5.2 Local: `ENVIRONMENT=prod node mcp-http.mjs` → exit 1 con mensaje en español
- [x] 5.3 Post-deploy pre-prod: CI configurado; smoke remoto (`curl /health`, MCP Inspector) al primer push a `preprod`
