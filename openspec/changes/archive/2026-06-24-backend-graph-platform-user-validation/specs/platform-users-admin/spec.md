...
`PATCH /api/platform/users/:id` (via `updatePlatformUser`) SHALL accept updates to `profile_code` and `is_active` only. It SHALL NOT accept, persist, or propagate changes to `email`. It SHALL NOT call Keycloak `updateUserEmail` or any Graph write operation. It SHALL NOT accept nor update `full_name`, `phone`, `rut_body`, or `rut_dv`.
...
- **AND** no call is made to Keycloak or Graph to change identity email
...
- **AND** does not show an editable Email field
