...
const LIST_PATH = '/app/gestion-contratos/templates-por-empresa'
...
export function CompanyTemplateViewPage() {
  const { id } = useParams()
  ...
  useEffect(() => {
    ...
    async function loadCompanyClauses() {
      ...
      const res = await fetchCompanyClausesList({ accessToken, companyId })
      if (!active || !res.ok) return
      const list = res.data?.items
  ...
  useEffect(() => {
    ...
    async function run() {
      if (!id || !accessToken || !companyId) {
        if (!companyId) {
          setLoading(false)
          setError('Seleccione una empresa en el subencabezado para ver la plantilla.')
          setEntity(null)
        }
        return
      }
      setLoading(true)
      setError(null)
      const res = await fetchCompanyTemplateById(id, { accessToken, companyId })
      if (!active) return
      setLoading(false)
      if (!res.ok) {
  ...
  const breadcrumb = useMemo(
    () => [
      { label: 'Templates por empresa', to: LIST_PATH },
      { label: 'Ver' },
    ],
    []
  )
          ...
          {loading ? (
            ...
            <>
              ...
              <div className="clause-form-row clause-form-row--editor">
                ...
                <RichTextEditor
                  key={`${entity.id}-${clauseOptions.length}-${companyClauseOptions.length}`}
                  readOnly
                  variant="document"
                  ...
                  enableEmbeddedCompanyClauses
                  embeddedCompanyClausesOptions={companyClauseOptions}
                  embeddedClauseCompanyId={companyId}
