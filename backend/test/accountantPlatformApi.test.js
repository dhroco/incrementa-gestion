...
test('POST /api/platform/accountants returns 403 when missing grant', async () => {
  const app = createApp({ requireAuth: authOk, requireGrant: () => grantForbidden() })
  const res = await request(app).post('/api/platform/accountants').send({ email: 'a@b.cl', full_name: 'Nombre' })
