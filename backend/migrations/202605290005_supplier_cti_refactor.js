/**
 * Refactor proveedores: STI (single table) → CTI (class table inheritance).
 * Reemplaza supplier monolítica por supplier + supplier_persona_natural + supplier_empresa.
 */
...
exports.up = async function up(knex) {
  ...
  await knex.schema.createTable('supplier_persona_natural', (table) => {
    table
      .uuid('supplier_id')
      .primary()
      .references('id')
    ...
    table.text('rut_body').notNullable()
    table.text('rut_dv').notNullable()
    table.text('address').nullable()
  })

  await knex.schema.createTable('supplier_empresa', (table) => {
    table
      .uuid('supplier_id')
      .primary()
      .references('id')
    ...
    table.text('rut_empresa_body').notNullable()
    table.text('rut_empresa_dv').notNullable()
  ...
  await knex.raw(`
    ...
    ADD CONSTRAINT supplier_empresa_personeria_type_check
    CHECK (
...
exports.down = async function down(knex) {
  await knex.schema.dropTableIfExists('supplier_social_network')
  await knex.schema.dropTableIfExists('supplier_empresa')
  await knex.schema.dropTableIfExists('supplier_persona_natural')
  await knex.schema.dropTableIfExists('supplier')
