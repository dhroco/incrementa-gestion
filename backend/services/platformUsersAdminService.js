...
const { getGraphClient } = require('../lib/graphClient')

const ADMIN_CLIENT_UNAVAILABLE_MSG =
  'El aprovisionamiento de usuarios no está configurado (falta configuración de Microsoft Graph).'
...
const IDP_LOOKUP_FAILED_MSG =
  'No se pudo consultar el directorio de Microsoft Entra. Intente nuevamente más tarde.'
...
async function createPlatformUser({ userId, payload }) {
  ...
  let entraUser
  try {
    entraUser = await graph.findUserByEmail(email)
  } catch (err) {
    return {
      ok: false,
      status: 503,
  ...
  const existingByEntraId = await db('user_profile').select('id').where({ user_id: entraUser.id }).first()
  ...
  try {
    await db.transaction(async (trx) => {
      ...
      const insertedRows = await trx('user_profile')
        .insert({
          user_id: entraUser.id,
          profile_id: profileId,
          email,
          full_name: entraUser.fullName,
          is_active: !!is_active
        })
        .returning('id')
